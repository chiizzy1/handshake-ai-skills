...
- Ads Creative relevance / Is the Generated Ad Still Relevant (original ad + one generated ad + ad text, Pass or Fail, Hero or Non-Hero type, check every failure reason that applies): use `handshake-ads-relevance` (read `../project-hedgehog/handshake-ads-relevance/SKILL.md`).
- Ads Creative text style / Compare Two Ad Overlays / Better Text Style (two finished ad images, judge only the overlaid text and graphics, ignore spelling and the picture, six buttons incl. "No overlaid text to compare"): use `handshake-ads-text-style` (read `../project-hedgehog/handshake-ads-text-style/SKILL.md`).
- Ads Creative text flaws / Compare Two Ad Images: Fewer Text Flaws (seed + creative direction + ad brief + two finished ads, judge only whether the overlaid text is correct, grounded in the seed or brief, and in the right language; ignore design and the picture; six buttons incl. "No overlaid text to compare"): use `handshake-ads-text-flaws` (read `../project-hedgehog/handshake-ads-text-flaws/SKILL.md`).
